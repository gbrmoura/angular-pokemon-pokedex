	...
	private loadPokemonStatistics(): void {
		...
		this.stats = this.pokemon.stats.map((stat: any) => {
			...
			return {
				...
				min:
					stat.stat.name == 'hp'
						? this.poke.getPokemonHPStat(stat.base_stat, 100)
						: this.poke.getPokemonOthersStats(false, stat.base_stat, 100),
				max:
					stat.stat.name == 'hp'
						? this.poke.getPokemonHPStat(stat.base_stat, 100, 31, 255)
						: this.poke.getPokemonOthersStats(true, stat.base_stat, 100, 31, 255),
			};
		});
	}
